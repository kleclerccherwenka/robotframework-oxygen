...
class RobotResultInterface(object):
    ...
    def spawn_robot_suite(self,
        ...
        if setup_keyword:
            robot_suite.setup = setup_keyword
        if teardown_keyword:
            robot_suite.teardown = teardown_keyword
    ...
    def spawn_robot_test(self,
        ...
        for keyword in keywords:
            if keyword:
                robot_test.body.append(keyword)
        if teardown_keyword:
            robot_test.teardown = teardown_keyword
    ...
    def build_keyword(self, starting_time, keyword, setup=False, teardown=False):
        ...
        keywords = tuple(keywords)  # required for nesting keywords
    ...
    def spawn_robot_keyword(self,
        ...
        if setup:
            ...
            keyword_type = BodyItem.TEARDOWN
        else:
            keyword_type = BodyItem.KEYWORD

        if status is None:
            keyword_status = 'NOT RUN'
        elif status:
            keyword_status = 'PASS'
        else:
            keyword_status = 'FAIL'
        ...
        for keyword in keywords:
            if keyword:
                robot_keyword.body.append(keyword)

        if teardown_keyword:
            robot_keyword.teardown = teardown_keyword
        ...
        for message in messages:
            if message:
                ...
                if ishtml:
                    message = message[6:]
                robot_keyword.body.create_message(message, html=ishtml)
